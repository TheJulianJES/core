...
async def test_missing_home_id_during_setup(
    hass: HomeAssistant, client: MagicMock
) -> None:
    """Test we retry setup when the server version message has no home ID."""
    client.connect.side_effect = KeyError("homeId")
    entry = MockConfigEntry(domain=DOMAIN, data={"url": "ws://test.org"})
    entry.add_to_hass(hass)
